...
int main(int argc, const char *argv[])
{
	...
	a64_jit_init(&jit, 34);
	a64_jit_push(&jit, a64_simd_umov(SIMD_B, 5, R0, R1));
	a64_jit_push(&jit, a64_simd_umov(SIMD_H, 5, R0, R1));
	a64_jit_push(&jit, a64_simd_umov(SIMD_S, 3, R0, R1));
	a64_jit_push(&jit, a64_simd_umov(SIMD_D, 0, R0, R1));

	a64_jit_push(&jit, a64_ret());

	out = fopen("files/umov.bin", "w");
